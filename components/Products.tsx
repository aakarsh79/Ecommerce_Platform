...
const Products = async ({ params, searchParams }: { params: { slug?: string[] }, searchParams: { [key: string]: string | string[] | undefined } }) => {
 ...
 // If out of stock checkbox is checked, stockMode is "lt"
  ...
  try {
    ...
    if (!data.ok) {
      // Silently handle API errors - don't throw to prevent error overlay
      products = [];
    } else {
    ...
    // Silently handle network errors - don't throw to prevent error overlay
    // The error overlay will show in dev, but we gracefully handle it
    products = [];
  }
