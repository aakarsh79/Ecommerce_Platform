  ...
  if(mode === "admin"){
      ...
      for (let i = 0; i < queryArray.length; i++) {
        ...
        if (queryParam.includes("filters") && filterType) {
          ...
          if (filterType === "category") {
            filterValue = queryParam.substring(queryParam.indexOf("=") + 1).toLowerCase(); // Normalize category name
          } else {
    ...
    if (Object.keys(filterObj).length === 0) {
      products = await prisma.product.findMany({
        ...
        take: 50, // Increased from 12 to 50 for better product display
        include: {
          category: {
            select: {
              name: true,
      ...
      if (filterObj.category && filterObj.category.equals) {
        // Increase take limit to get more products per category
        const takeLimit = 50; // Get up to 50 products per page for category filters
        products = await prisma.product.findMany({
          skip: (validatedPage - 1) * 10,
          take: takeLimit,
          include: {
          ...
          where: {
            ...
            category: {
              name: {
                equals: filterObj.category.equals, // Already normalized when extracted
              },
            },
          },
          orderBy: sortObj,
